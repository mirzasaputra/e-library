...
<script>
    ...
    async function onScanSuccess(decodedText, decodedResult) {
        if(typeof decodeText != 'undefined' && !close || decodedText != null && !close){
            close = true
            swal.fire({
                title: 'Please Wait',
                html: 'Processing',
            ...
            const res = await fetch(`${$('meta[name="base-url"]').attr('content')}/administrator/book-returns/${decodedText}/scanner`)
    
            swal.close()
            if(res.status == 200){
                var data = await res.json()
                pushState(`${$('meta[name="base-url"]').attr('content')}/administrator/book-returns/${data.hashid}/show`)
            } else {
                swal.fire({
                    title: 'Peringatan',
                    icon: 'warning',
